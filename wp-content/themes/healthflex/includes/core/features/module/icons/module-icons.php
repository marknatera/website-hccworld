...
if ( !class_exists('Plethora_Module_Mediapanel') ) {
	...
	class Plethora_Module_Icons {
		...
		public function __construct() {
			...
			// Dev demo ( will be removed )
			add_shortcode( SHORTCODES_PREFIX . 'iconsdemo', array( $this, 'demo_shortcode' ) );
		}
		...
		public function diagnostics_check_version() {
			...
			if ( $check_version === 'v1' ) { 
				...
				$notice .= '</p>';
				// $notice .= '<a href="'.admin_url( 'admin.php/?page='.THEME_OPTVAR.'') .'&plethora_module_icons_diagnostics_v2=hide">'. esc_html__( 'Dismiss this notice', 'plethora-framework' ) .'</a>';
				$notice .= '</div>';
...
// Admin User Interaction Methods ---> START
	    ...
	    public function theme_options_tab( $sections ) { 
	    	...
	    	// $desc .= '<br><a href="#TB_inline?width=900&height=550&inlineId=plethora-module-icons-preview-id" class="thickbox">'. __('Please read this before start working with icon libraries', 'plethora-framework')  .'</a>.' ;
		...
		public function get_preset_libraries_desc() {
			...
			$subtitle_text .= '<li><a href="http://linea.io/" target="_blank">Linea Basic</a></li>';
		...
		*/
		public function vc_icon_append_libraries_to_dropdown() {
			...
			if ( Plethora_Theme::is_library_active() ) { 
				...
				if ( function_exists('vc_update_shortcode_param') && !empty( $param_type ) ) {
		...
		public function vc_icon_add_libraries_iconpickers( ){
			...
			if ( Plethora_Theme::is_library_active() ) { 
				...
				if ( function_exists('vc_add_params') && !empty( $params ) ) {
...
// PUBLIC Methods to be used outside class  ---> START
		...
		*/
		...
		public function demo_shortcode( $atts, $content = null ) { 
          ...
          extract( shortcode_atts( array( 
            ...
            preg_match('/^((?P<whole>\d+)(?=\s))?(\s*)?(?P<numerator>\d+)\/(?P<denominator>\d+)$/', $cols, $fraction);
            $sum_numdenom = ( $fraction['numerator'] != 0 ) ? $fraction['numerator'] / $fraction['denominator'] : 0;
            $decimal_width = $fraction['whole'] + $sum_numdenom;
            $colsize = floor( $decimal_width * 12 );

			$libraries = self::get_libraries();
			$libraries = !empty( $library ) && isset( $libraries[$library] ) ? array( $libraries[$library] ) : $libraries;
			$output = '';
			foreach ( $libraries as $key => $args ) { 
				if ( $args['status'] && ! $admin ) {
					$library_icons = $args['icons'];
					if ( $title ) { 
						$output .= '<div class="col-md-12 section_header fancy centered margin_top">';
						$output .= '<h6>'. $args['title'] .' ( '. count($library_icons) .' icons )</h6>';
						$output .= '</div>';
					}
					asort( $library_icons );
					foreach ( $library_icons as $key=>$icon ) { 
						$output .= '<div class="col-md-'. $colsize .'">';
						$output .= '<a href="#" style="line-height:40px;">';
						$output .= Plethora_Module_Icons::get( array( 'icon' => $icon['class'], 'style' => array('width:36px', 'font-size:16px'), 'demo' => true ) );
						$output .= '</a></div>';

					}
				} elseif ( $args['status'] && $admin ) {

					$library_icons = $args['icons'];
					if ( $title ) { 
						$output .= '<h1 style="margin-top:40px; margin-bottom:20px;">'. $args['title'] .' ( '. count($library_icons) .' icons )</h1>';
					}
					asort( $library_icons );
					foreach ( $library_icons as $key=>$icon ) { 
						$output .= '<div style="padding: 10px; margin-bottom:10px; background-color:#53BDBD; border-radius: 5px; color:#fff;">';
						$output .= '<div style="display:inline-block; text-align:center; margin-right:10px;">';
						$output .= Plethora_Module_Icons::get( array( 'icon' => $icon['class'], 'style' => array('width:28px', 'font-size:22px'), 'demo' => false ) );
						$output .= '</div>';
						$output .= '<div style="line-height:24px; display:inline-block; font-size:15px; font-weight:bold;">';
