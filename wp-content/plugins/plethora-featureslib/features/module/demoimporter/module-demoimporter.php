...
//// UI METHODS START
    ...
    public function ui_header() {
      ...
      $header .= '<p>'. esc_html__('Preview and select a demo to import. We always recommend importing only one demo at a single installation. Before trying to import the demo of your choice, make sure that you have installed / activated all required plugins .  ') .'</p>';
      return $header;
    }
      ...
      foreach ( $plugins_required as $plugin_slug => $plugin ) {
        ...
        if ( is_plugin_active( $plugin_slug ) ) { 
          ...
          if ( array_key_exists( $plugin_slug, $plugins_installed ) ) { 
            ...
            $plugin_info .= '<div><a href="'. admin_url( 'plugins.php' ) .'" target="_blank" style="color:darkred">'. $plugin->Name .' ( by '. wp_strip_all_tags( $plugin->Author ) .' )</a><span>You have to activate it before importing this demo.</span></div>';

          } else {

            $plugin_info .= '<div><a href="'. admin_url( 'plugin-install.php?tab=search' ).'" target="_blank" style="color:red">'. $plugin->Name .' ( by '. wp_strip_all_tags( $plugin->Author ) .' )</a><span>You have to install AND activate it before importing this demo.</span></div>';
          }
        }
      }
